...
class Model extends Pdox
{
    ...
    public function __construct()
    {
        ...
        parent::__construct([
            'host'      => Helper::config('database.host'),
            'driver'    => Helper::config('database.driver'),
            'database'  => Helper::config('database.name'),
            'username'  => Helper::config('database.user'),
            'password'  => Helper::config('database.pass'),
            'charset'   => Helper::config('database.charset'),
            'collation' => Helper::config('database.collation'),
            'prefix'    => Helper::config('database.prefix'),
    ...
    public function dbInit($schema): bool
    {
        ...
        // delete other tables
        $sql = "SELECT CONCAT(`TABLE_NAME`) FROM information_schema.TABLES WHERE TABLE_SCHEMA = \"" . Helper::config('database.name') . "\"";
    ...
    public function dbSeed($schema): bool
    {
    ...
    public function cache($time)
    {
        if (Helper::config('settings.db_cache')) {
            ...
            if (!is_dir($dir = Helper::path('app/Storage')))
                mkdir($dir);
