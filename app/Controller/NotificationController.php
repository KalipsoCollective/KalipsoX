<?php

/**
 * @package KX
 * @subpackage Controller\NotificationController
 */

declare(strict_types=1);

namespace KX\Controller;

use KX\Core\Controller;
use KX\Core\Helper;
use KX\Model\Notifications as NotificationsModel;
use KX\Model\EmailLogs as EmailLogsModel;
use PHPMailer\PHPMailer\PHPMailer;
use PHPMailer\PHPMailer\Exception as PHPMailerException;

final class NotificationController extends Controller {

    /**
     * Notification types from External/notifications
     */
    public $types;

    /**
     * Factory class 
     */
    public $container;

    /**
     * Email and Notifications Models
     **/
    public $emailModel;
    public $notificationsModel;

    /**
     * @param object $container   It must be a factory object. 
     * @return void
     */
    public function __construct ($container) {

        if (file_exists($file = Helper::path('app/External/notifications.php')))
            $this->types = require $file;
        else
            throw new \Exception((string) Helper::lang('error.notification_hook_file_not_found'));

        $this->container = $container;
        $this->emailModel = (new EmailLogsModel());
        $this->notificationsModel = (new NotificationsModel());

    }

    public function add($type, $data = null) {

        if (isset($this->types[$type]) !== false) {

            return $this->types[$type]($this, $data);

        }
        
    }

    /**
     * 
     * @param array $arguments  email content 
     * @return boolen|integer 
     **/
    public function addEmail($arguments) {

        $title = $arguments['title'];
        $app = Helper::config('settings.name');
        $body = $arguments['body'];

        if (file_exists($template = Helper::path('app/External/email_template.html'))) { // with template

            $footer = $app . ' (c) ' . date('Y');

            $content = str_replace([
                '[TITLE]',
                '[ALT_BODY]',
                '[APP]',
                '[BODY]',
                '[FOOTER]'
            ], [
                $title,
                trim(strip_tags($body)),
                $app,
                $body,
                $footer
            ], file_get_contents($template));

        } else {
            $content = $body;
        }

        $status = 'pending';
        if (! Helper::config('settings.mail_queue')) { // Direct sending

            $status = $this->sendEmail($arguments['recipient_email'], $arguments['recipient'], $content, $title) ? 
                'completed' : 'uncompleted';

        }

        if (! is_dir($path = Helper::path('app/Storage'))) mkdir($path);
        if (! is_dir($path .= '/email')) mkdir($path);
        if (! is_dir($path .= '/' . $status)) mkdir($path);

        $date = time();

        $path .= '/' . $date . '.html';

        file_put_contents($path, $content);

        return $this->emailModel->insert([
                'date'          => $date,
                'email'         => $arguments['recipient_email'],
                'name'          => $arguments['recipient'],
                'title'         => $arguments['title'],
                'user_id'       => $arguments['recipient_id'],
                'sender_id'     => (isset($arguments['sender_id']) !== false ? $arguments['sender_id'] : null),
                'file'          => $date . '.html',
                'status'        => $status
            ]);

    }

    public function sendEmail($recipientMail, $recipientName = '', string $content = '', string $title = '') {

        $return = false;

        if (Helper::config('app.dev_mode')) $recipientMail = Helper::config('settings.contact_email');
        
        $sendingType = Helper::config('settings.mail_send_type');

        if (Helper::config('app.dev_mode')) {
            $sendingType = 'server';
        }

        switch ($sendingType) {

            case 'smtp':
                $mail = new PHPMailer(true);
                if ($lang = Helper::lang('lang.code') !== 'en') 
                    $mail->setLanguage($lang);

                try {
                    $mail->SMTPDebug = 0;
                    $mail->isSMTP();
                    $mail->Host         = Helper::config('settings.smtp_address');
                    $mail->SMTPAuth     = true;
                    $mail->Username     = Helper::config('settings.smtp_email_address');
                    $mail->Password     = Helper::config('settings.smtp_email_pass');
                    $mail->SMTPSecure   = PHPMailer::ENCRYPTION_STARTTLS;
                    $mail->Port         = Helper::config('settings.smtp_port');
                    $mail->CharSet      = Helper::config('app.charset');

                    $reply = Helper::config('settings.contact_email');
                    if (! $reply OR $reply == '') {
                        $reply = Helper::config('settings.smtp_email_address');
                    }
                    //Recipients
                    $mail->setFrom(Helper::config('settings.smtp_email_address'), Helper::config('app.name'));
                    $mail->addAddress($recipientMail, $recipientName);      // Add a recipient
                    $mail->addReplyTo($reply, Helper::config('app.name') );

                    // Content
                    $mail->isHTML(true);                                    // Set email format to HTML
                    $mail->Subject = $title;
                    $mail->Body    = $content;
                    $mail->AltBody = trim(strip_tags($content));

                    if ($mail->send()) {
                        $return = true;
                    } else {
                        $return = false;
                    }

                } catch (PHPMailerException $e) {

                    $return = false; // $e->errorMessage();
                }
                break;
            
            default:
                $headers = "Reply-To: ". Helper::config('settings.contact_email') . "\r\n";
                $headers .= "MIME-Version: 1.0\r\n";
                $headers .= "Content-Type: text/html; charset='".Helper::config('app.charset')."'\r\n";
                $return = mail($recipientMail, $title, $content, $headers);
                break;
        }

        return $return;

    }


    /**
     * It sends the emails waiting in the queue based on the limit.
     * @param int $limit        shooting lmit
     * @return array $return    report
     */
    public function mailQueue(int $limit = 10) {

        $return = [];
        $model = new EmailLogsModel;
        $queue = $model->select('id, date, email, name, title, file, status')
            ->where('status', 'pending')
            ->orderBy('id', 'desc')
            ->limit($limit)
            ->getAll();

        if (! empty($queue)) {

            foreach ($queue as $mail) {

                $move = false;

                $file = Helper::path('app/Storage/email/' . $mail->status . '/' . $mail->file);
                if ($mail->date > strtotime('-3 days') AND file_exists($file)) {

                    $content = file_get_contents($file);
                    $send = $this->sendEmail($mail->email, $mail->name, $content, $mail->title);
                    if ($send) {
                        $move = 'completed';
                    } else {
                        $move = 'uncompleted';
                    }

                } else {
                    $move = 'uncompleted';
                }

                $return[$mail->id] = '';

                if ($move) {
                    if (file_exists($file)) {

                        if (! is_dir($newDir = Helper::path('app/Storage/email/' . $move)))
                            mkdir($newDir);

                        if (rename($file, $newDir . '/' . $mail->file)) {
                            $return[$mail->id] .= 'Moved to: ' . $move . '/' . $mail->file . PHP_EOL;
                        }
                    }

                    if ($model->where('id', $mail->id)->update(['status' => $move])) {
                        $return[$mail->id] .= 'Updated as: ' . $move;
                    }

                }

            }
        }

        return $return;
    }

}